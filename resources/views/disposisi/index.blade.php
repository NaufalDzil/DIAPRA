                    ...
                    @else
                        @foreach ($disposisi as $key => $row)
                            <tr class="text-center">
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $row->tujuan }}</td>
                                <td class="text-center">@if($row->disposisi)
                                    <a href="{{ route('disposisi.surat', ['suratmasuk' => $suratmasuk->id, 'disposisi' => $row->id]) }}" class="btn btn-outline-success btn-sm">
                                        <i class="far fa-file-pdf"></i> Lembar Disposisi
                                    </a>@endif
                                    <a href="{{ route('disposisi.edit', ['suratmasuk' => $suratmasuk->id, 'disposisi' => $row->id]) }}" class="btn btn-outline-danger btn-sm"><i class="fas fa-edit"></i> Edit</a>
                                </td>
                            </tr>
                        @endforeach
                    @endif
